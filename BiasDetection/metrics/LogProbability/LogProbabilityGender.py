...
class LogProbabilityGender(LogProbability):
    ...
    def attention_mask_creator(self, input_ids):
    ...
    def tokenize_to_id(self, sentences, tokenizer):
    ...
    def input_pipeline(self, sequence, tokenizer, MAX_LEN):
        """function to tokenize, pad and create attention masks"""
        input_ids = self.tokenize_to_id(sequence, tokenizer)
        ...
        attention_masks = self.attention_mask_creator(input_ids)

        return input_ids, attention_masks


    def prob_with_prior(self, pred_TM, pred_TAM, input_ids_TAM, original_ids, tokenizer):
        pred_TM = pred_TM.cpu()
    ...
    def LogProbabilityBec(self, model, tokenizer, device):
        ...
        eval_tokens_TM, eval_attentions_TM = self.input_pipeline(eval_df.Sent_TM,
                                                            tokenizer,
                                                            max_len_eval)
        eval_tokens_TAM, eval_attentions_TAM = self.input_pipeline(eval_df.Sent_TAM,
                                                            tokenizer,
                                                            max_len_eval)
        eval_tokens, _ = self.input_pipeline(eval_df.Sentence, tokenizer, max_len_eval)
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            associations = self.prob_with_prior(predictions_TM,
                                        predictions_TAM,
                                        b_input_TAM,
